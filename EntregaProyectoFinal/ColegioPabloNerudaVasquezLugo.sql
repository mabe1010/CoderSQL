...
DELIMITER //
CREATE FUNCTION `Ingresos`(Mes ENUM("Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre"), Anio YEAR) RETURNS float
    READS SQL DATA
BEGIN
	DECLARE Total FLOAT;
...
-- Funcion para insertar y eliminar un valor:
DELIMITER //
CREATE FUNCTION `PromedioNotaPorAlumno`(Id INT, Lapso INT, Anio YEAR) RETURNS int
    READS SQL DATA
BEGIN
...
CREATE USER 'Directivo'@'localhost';
SELECT * FROM USER;

-- Definicion de contraseñas
ALTER USER 'Admin'@'localhost' IDENTIFIED BY 'Neruda2021';
ALTER USER 'Directivo'@'localhost' IDENTIFIED BY 'NerudaProf2021';
SELECT * FROM USER;

-- Definición de permisos a cada usuario:
-- Pemiso de solo lectura/consulta para el usuario administrativo para todas las tablas:
GRANT SELECT ON *.* TO 'Admin'@'localhost';

-- Permisos para lectura/consulta, actualización e inserción de datos para el usuario administrativo para todas las tablas:
GRANT SELECT, INSERT, UPDATE ON *.* TO 'Directivo'@'localhost';

-- Consultamos la base de usuarios para validar los permisos aplicados:
SHOW GRANTS FOR 'Admin'@'localhost';
SHOW GRANTS FOR 'Directivo'@'localhost';

-- Eliminar registro iniciando previamente una trnsacción:
START TRANSACTION;
	DELETE FROM colegio_pablo_neruda.factregistronotas
	WHERE IdNotaFinal = 4;
ROLLBACK; -- Vuelta atrás de la transacción
commit; -- confirmación del cambio

-- inserción de nuevos registros iniciando una transacción:
START TRANSACTION;
	INSERT INTO colegio_pablo_neruda.factmensualidad VALUES(NULL, 1, '2021-12-29', 2021, 'Diciembre', 20000.00, 'Deposito', 1);
    INSERT INTO colegio_pablo_neruda.factmensualidad VALUES(NULL, 2, '2021-12-28', 2021, 'Diciembre', 20000.00, 'Tarjeta', 1);
    INSERT INTO colegio_pablo_neruda.factmensualidad VALUES(NULL, 3, '2021-12-29', 2021, 'Diciembre', 20000.00, 'Efectivo', 1);
    INSERT INTO colegio_pablo_neruda.factmensualidad VALUES(NULL, 4, '2021-12-29', 2021, 'Diciembre', 20000.00, 'Deposito', 1);
SAVEPOINT lote_1_4;
    INSERT INTO colegio_pablo_neruda.factmensualidad VALUES(NULL, 7, '2021-12-29', 2021, 'Diciembre', 20000.00, 'Efectivo', 1);
    INSERT INTO colegio_pablo_neruda.factmensualidad VALUES(NULL, 8, '2021-12-29', 2021, 'Diciembre', 20000.00, 'Deposito', 1);
    INSERT INTO colegio_pablo_neruda.factmensualidad VALUES(NULL, 10, '2021-12-29', 2021, 'Diciembre', 20000.00, 'Efectivo', 1);
	INSERT INTO colegio_pablo_neruda.factmensualidad VALUES(NULL, 12, '2021-12-29', 2021, 'Diciembre', 20000.00, 'Efectivo', 1);
SAVEPOINT lote_5_8;
-- Eliminar el savepoint de las primeras cuatro inserciones: 
